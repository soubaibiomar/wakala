...
# Define the models
llm = ChatOpenAI(model=settings.LLM_MODEL, openai_api_key=settings.OPENAI_API_KEY, temperature=0.7)
...
RÈGLE ABSOLUE : Si l'utilisateur cherche une voiture, NE LUI RECOMMANDE QUE LES VÉHICULES FOURNIS DANS LE CONTEXTE CI-DESSOUS.
...
async def analyze_intent(user_message: str) -> Dict[str, Any]:
    ...
    analysis_prompt = ChatPromptTemplate.from_messages([
        ("system", "Tu es un analyseur d'intention. Retourne UNIQUEMENT un objet JSON valide avec les clés : 'intent' (valeurs possibles: 'car_search', 'general_advice', 'customs') et 'max_price' (entier ou null si aucun budget max n'est mentionné). N'ajoute pas de texte autour du JSON."),
        ("human", "{message}")
    ])
    
    # We use a secondary LLM call with JSON mode to reliably get the intent
    analyzer_llm = ChatOpenAI(model=settings.LLM_MODEL, openai_api_key=settings.OPENAI_API_KEY, temperature=0).bind(
...
async def chat_stream(message: str, history: List[Dict[str, str]]) -> AsyncIterable[str]:
    ...
    if intent == "car_search":
        context = await retrieve_vehicles(message, max_price=max_price)
