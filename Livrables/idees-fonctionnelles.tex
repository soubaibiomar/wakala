...
\title{
Référentiel des Idées Fonctionnelles\\Projet Vente Auto Platform}
...
\subsection{1. Chatbot (Assistant Virtuel)}
\textbf{Catégorie :} Expérience B2C

\textbf{Explication}\\
Le Chatbot agit comme un conseiller virtuel expert pour les acheteurs. Au lieu d'une simple recherche par filtres, l'utilisateur peut exprimer son besoin naturellement (ex. : "Quels SUV consomment moins de 5L/100km et sont disponibles près de Lyon ?"). Le système comprend la phrase, cherche les véhicules pertinents et discute avec l'utilisateur pour l'orienter. 

\textbf{Détails et Cas d'Usage :} 
Si un acheteur novice dit \textit{"Je veux une voiture familiale robuste, budget 120 000 DH"}, le Chatbot va :
1. Comprendre l'intention (Famille = 5 places min, grand coffre / Robuste = fiabilité).
2. Poser une question de précision si nécessaire : \textit{"Faites-vous plutôt de l'autoroute ou de la ville pour vous orienter sur du Diesel ou de l'Hybride ?"}
3. Expliquer ses choix : \textit{"Je vous suggère cette Honda Civic car elle est réputée pour ne jamais tomber en panne."}
Toute la complexité technologique est masquée pour offrir une discussion fluide et humaine.

\subsection{2. Moteur de Recommandation}
\textbf{Catégorie :} Expérience B2C

\textbf{Explication}\\
Ce système va bien au-delà de la simple suggestion de voitures similaires. Il agit comme un véritable entremetteur (Matchmaker) qui comprend les goûts de l'utilisateur et observe ce que les autres acheteurs ayant le même profil ont finalement choisi. Le but est de proposer "LA" bonne voiture, même si l'acheteur n'avait pas initialement pensé à ce modèle précis.

\textbf{Détails de la Logique de Scoring :}
La recommandation s'appuie sur plusieurs critères qui sont pondérés en temps réel :
- \textbf{Affinité du profil :} Si le client regarde beaucoup de voitures avec des toits ouvrants, le moteur pousse des modèles similaires vers le haut de la liste.
- \textbf{Besoins explicites :} Respect strict du budget et de la capacité (ex: coffre).
- \textbf{Sérendipité (Effet de surprise) :} Le moteur introduit 1 ou 2 alternatives audacieuses mais très pertinentes (ex: suggérer un break très logeable et économique à quelqu'un qui cherchait absolument un SUV).
...
\textbf{Explication}\\
En complément du Chatbot, l'utilisateur tape simplement ce qu'il veut dans une barre de recherche globale. Le système comprend les concepts flous ("voiture pour jeune permis pas chère") et renvoie les résultats pertinents en classant par pertinence sémantique, plutôt que de chercher des correspondances exactes de mots.
...
\section{Conclusion}
Ce référentiel illustre l'ambition fonctionnelle de la plateforme "Vente Auto Platform". Le projet repose sur deux piliers principaux : un Chatbot qui écoute et accompagne, et un Moteur de Recommandation qui trouve la perle rare. En simplifiant l'accès au marché automobile grâce à l'Intelligence Artificielle, le projet dépasse le simple site de petites annonces pour devenir un véritable assistant d'achat personnel.
