...
class ExcalidrawGenerator:
    ...
    def add_rectangle(self, x, y, width, stroke_color, bg_color, text_title, routes=None, text_body=None):
        rect_id = generate_id()
        group_id = generate_id()
        
        # Calculate dynamic height
        routes_h = len(routes) * 20 if routes else 5
        body_h = (text_body.count('\n') + 1) * 20 if text_body else 0
        ...
        title_el = self.add_text(text_title, title_x, y + 12, font_size=16, color="#1e1e1e", align="center", bold=True)
        ...
        if text_body:
            ...
            body_el = self.add_text(text_body, body_x, y + current_y_offset + 5, font_size=13, color="#495057", align="center")
            body_el["groupIds"] = [group_id]
            
        return rect_id, box_h
...
def build_flow(gen, start_y, title, subtitle, steps, theme):
    ...
    for i, step in enumerate(steps):
        # Draw main box with dynamic height
        _, box_h = gen.add_rectangle(box_x, current_y, box_w, theme[0], theme[1], step["title"], step.get("routes"), step["body"])
        
        # Draw explanation on right
        if step.get("explanation"):
            gen.add_text(step["explanation"], text_x, current_y + (box_h / 2) - 15, font_size=14, color="#495057", align="left")
...
gen.generate(r"d:\Projet automobile\vente-auto-platform\Livrables\wakala_detailed_flows_v5.excalidraw")
print("Excalidraw v5 file generated successfully.")
