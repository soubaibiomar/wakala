...
class SelectorRepair:
    ...
    def propose_repair(self, site: str, field: str, html_snippet: str) -> Optional[str]:
        """
        Handles Structural Drift by querying Ollama for a new CSS selector
        based on the provided HTML snippet.
        Only called on branch (a) Structural Drift. NEVER on Active Block.
        """
        logger.info(f"Initiating Ollama self-repair for {site}.{field} (Structural Drift)")
        ...
        new_selector = ask_ollama(prompt)
        
        if not new_selector:
            logger.error(f"Ollama failed to propose a new selector for {site}.{field}")
            return None
            
        logger.info(f"Ollama proposed new selector: {new_selector}")
