...
platforms = [
    {"name": "wandaloo", "base_url": "https://www.wandaloo.com/occasion/"},
    {"name": "leguideauto", "base_url": "https://www.leguideauto.ma/occasion/"},
    {"name": "carz", "base_url": "https://www.carz.ma/occasion/"},
    {"name": "kifal", "base_url": "https://kifal-auto.ma/achat-voiture-occasion"},
    {"name": "otoclic", "base_url": "https://www.otoclic.ma/voitures-occasion"},
    {"name": "global_occaz", "base_url": "https://globaloccaz.ma/vehicules"},
    {"name": "spoticar", "base_url": "https://www.spoticar.ma/voitures-occasion"}
]

base_dir = r"d:\Projet automobile\vente-auto-platform\data_pipeline\kafka\producers\scrapers"

scraper_template = """from datetime import datetime
...
yaml_template = """version: "2026-07-20"
...
fields:
  ...
  transmission: ".transmission"
  body_type: ".body"
  is_inspected: ".inspected-badge"
  inspection_points: ".inspection-points"
  has_warranty: ".warranty-badge"
  warranty_months: ".warranty-duration"
  is_certified: ".certified-badge"
fallback_fields:
  listing_card: [".card", "article"]
  url: ["a"]
  price: [".price-tag", "span.price"]
  title: ["h3", "h4", "h2"]
"""

for p in platforms:
    class_name = "".join([word.capitalize() for word in p["name"].split("_")])
    
    # Scraper
    scraper_path = os.path.join(base_dir, f"{p['name']}_scraper.py")
    with open(scraper_path, "w", encoding="utf-8") as f:
        f.write(scraper_template.format(
    ...
    # Yaml
    yaml_path = os.path.join(base_dir, "selectors", f"{p['name']}_selectors.yaml")
